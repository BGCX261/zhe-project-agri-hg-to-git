...
require_once './libs/check_user_privileges.lib.php';
 ...
 */
require_once('./libs/parse_analyze.lib.php');
...
if (isset($GLOBALS['show_as_php']) || !empty($GLOBALS['validatequery'])) {
    ...
    if (isset($purge) && $purge == '1') {
        require_once './libs/relation_cleanup.lib.php';
...
if ($num_rows < 1 || $is_affected) {
    ...
    if ($is_gotofile) {
        ...
        include 'libs/db_table_exists.lib.php';
...
// At least one row is returned -> displays a table with results
    ...
    if (isset($printview) && $printview == '1') {
        require_once './libs/header_printview.inc.php';
    } else {
        $js_to_run = 'functions.js';
        unset($message);
        if (isset($table) && strlen($table)) {
            require './libs/tbl_common.php';
            $url_query .= '&amp;goto=tbl_sql.php&amp;back=tbl_sql.php';
            require './libs/tbl_info.inc.php';
            require './libs/tbl_links.inc.php';
        } elseif (isset($db) && strlen($db)) {
    ...
    require_once './libs/display_tbl.lib.php';
    if (empty($disp_mode)) {
        // see the "PMA_setDisplayMode()" function in
        // libs/display_tbl.lib.php
