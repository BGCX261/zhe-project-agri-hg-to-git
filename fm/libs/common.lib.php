 ...
 *
 * the include of libs/defines_mysql.lib.php must be after the connection
 * to db to get the MySql version
 *
 * the authentication libs must be before the connection to db
 *
 ...
 * - db connection
 * - authentication work
 * - load of the libs/defines_mysql.lib.php library to get the MySQL
 *   release number
 */
...
/**
 * include here only libs which contain only function definitions
 * no code im main()!
 */
/**
 * Input sanitizing
 */
require_once './libs/sanitizing.lib.php';
/**
 * the PMA_Theme class
 */
require_once './libs/Theme.class.php';
/**
 * the PMA_Theme_Manager class
 */
require_once './libs/Theme_Manager.class.php';
/**
...
if (!defined('PMA_MINIMUM_COMMON')) {
     ...
     */
    require_once './libs/js_escape.lib.php';
    ...
    function PMA_whichCrlf()
    {
        ...
        // The 'PMA_USR_OS' constant is defined in "./libs/defines.lib.php"
        // Win case
        if (PMA_USR_OS == 'Win') {
            $the_crlf = "\r\n";
        }
...
//]]>
    ...
    function PMA_checkParameters($params, $die = true, $request = true)
    {
        ...
        if ($found_error) {
             ...
             */
            require_once './libs/header_meta_style.inc.php';
     ...
     */
    require_once './libs/url_generating.lib.php';
...
if (empty($__redirect) && !defined('PMA_NO_VARIABLES_IMPORT')) {
    require './libs/grab_globals.lib.php';
}
...
if ((isset($_REQUEST['token']) && !is_string($_REQUEST['token'])) || empty($_REQUEST['token']) || $_SESSION[' PMA_token '] != $_REQUEST['token']) {
     ...
     */
    require_once('./libs/cleanup.lib.php');
    /**
     * Do actual cleanup
     */
...
if (! defined('PMA_MINIMUM_COMMON')) {
     ...
     * Charset conversion.
     */
    require_once './libs/charset_conversion.lib.php';

    /**
     * String handling
     */
    require_once './libs/string.lib.php';
    ...
    if (! empty($cfg['Server'])) {
         ...
         */
        require_once './libs/database_interface.lib.php';
        ...
        if (!file_exists('./libs/auth/' . $cfg['Server']['auth_type'] . '.auth.lib.php')) {
            header('Location: error.php'
                    . '?lang='  . urlencode($available_languages[$lang][2])
                    . '&dir='   . urlencode($text_dir)
                    . '&type='  . urlencode($strError)
         ...
         */
        require_once './libs/auth/' . $cfg['Server']['auth_type'] . '.auth.lib.php';
        ...
        if (isset($cfg['Server']['AllowDeny'])
          && isset($cfg['Server']['AllowDeny']['order'])) {
             ...
             */
            require_once './libs/ip_allow_deny.lib.php';
         ...
         * SQL Parser code
         */
        require_once './libs/sqlparser.lib.php';

        /**
         * SQL Validator interface code
         */
        require_once './libs/sqlvalidator.lib.php';

        /**
         * the PMA_List_Database class
         */
        require_once './libs/PMA_List_Database.class.php';
    ...
    if (@function_exists('mb_convert_encoding')
        && strpos(' ' . $lang, 'ja-')
        && file_exists('./libs/kanji-encoding.lib.php')) {
        require_once './libs/kanji-encoding.lib.php';
