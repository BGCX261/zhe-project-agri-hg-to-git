...
class Coaadmin extends CI_Controller {
...
function test() {
	echo $this->encrypt->encode('Duboc85o');
}
...
function addBanner() {
	...
	header('location:/coaadmin/mngBanner');
	return;
}
...
function vcatalog() {
	if($this->session->userdata('login')) {
		...
		if($addVC) {
			...
			header('location:/coaadmin/vcatalog?'.rand());
			return;
		}
...
function addVideo() {
	...
	if(isset($_POST['videocatalog_id'])) {
		...
		if($newid) {
			...
			header('location:/coaadmin/addVideoSecc/'.$newid.$resultmsg.$bigimgmsg);
			return;
		}
...
function deleVideo($vid = null, $vcid = null) {
	...
	if($vid) {
		...
		if($from ==='search') {
			header('location:/coaadmin/searchVideo?q='.$this->input->get('q').'&p='.$this->input->get('p').'&l='.$this->input->get('l'));
		} else {
			header('location:/coaadmin/mngVideo/'.$vcid);
		}
	} else {
		header('location:/coaadmin');
	}
...
function addEbook() {
	...
	if(isset($_POST['title'])) {
		...
		if($newid) {
			...
			header('location:/coaadmin/addEbookSecc/'.$newid.$resultmsg.$bigimgmsg.$pdfmsg);
			return;
		}
...
function deleEbook($eid = null) {
	...
	if($eid) {
		...
		header('location:/coaadmin/mngEbook');
	} else {
		header('location:/coaadmin');
	}
}
